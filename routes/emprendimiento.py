...
def resumen(eid):
    ...
    if request.method == 'POST' and 'monto' in request.form:
        ...
        try:
            ...
            # Insertar movimiento
            cursor.execute("""
                INSERT INTO movimientos_emprendimiento (emprendimiento_id, fecha, concepto, detalle, monto, usuario_id, producto_id)
    ...
    # A. Totales para las tarjetas superiores
    cursor.execute("""
        SELECT 
    ...
    # B. Datos para la Gráfica de Ventas Diarias (Línea)
    # Agrupamos por fecha para que el gráfico de líneas tenga sentido
    cursor.execute("""
    ...
    """, (eid, current_user.id))
    res_grafico = cursor.fetchall()
    
    # Formateamos las fechas para que JS las entienda (dd/mm)
    labels_ingresos = [r['fecha'].strftime('%d/%m') if hasattr(r['fecha'], 'strftime') else str(r['fecha']) for r in res_grafico]
    valores_ingresos = [float(r['total']) for r in res_grafico]

    # C. Lista de Movimientos para la tabla (con el filtro que pide tu HTML)
    ...
    # D. Productos para el selector (Importante: nombre 'productos_lista')
    cursor.execute("SELECT id, nombre, detalle, talle, stock FROM productos WHERE emprendimiento_id=%s AND usuario_id=%s", (eid, current_user.id))
    ...
    return render_template(
        ...
        productos_lista=productos_lista, # Sincronizado con el for prod in productos_lista
        desde=desde,
        hasta=hasta,
