...
def eliminar_movimiento(mid, eid):
    conn = get_db()
    conn.execute(
        "DELETE FROM movimientos_emprendimiento WHERE id=? AND usuario_id=?", 
        (mid, current_user.id)
    )
    conn.commit()
    return redirect(url_for('emprendimiento.resumen', eid=eid))
...
def eliminar_producto(pid, eid):
    ...
    conn.execute(
        "DELETE FROM productos WHERE id=? AND emprendimiento_id=? AND usuario_id=?", 
        (pid, eid, current_user.id)
    )
    conn.commit()
